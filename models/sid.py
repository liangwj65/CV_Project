...
class MFViT(nn.Module):
    ...
    def __init__(
        ...
        elif isinstance(self.vit, (backbones.CLIPBackbone, CLIPBackboneWithSVD)):
            # CLIP normalization (支持原始CLIPBackbone和带SVD的CLIPBackboneWithSVD)
            self.backbone_norm = transforms.Normalize(
    ...
    def export_onnx(self, export_file: pathlib.Path) -> None:
        ...
        class ExportableMFVit(nn.Module):
            def __init__(self):
                ...
                elif isinstance(outer_instance.vit, (backbones.CLIPBackbone, CLIPBackboneWithSVD)):
                    # CLIP normalization (支持原始CLIPBackbone和带SVD的CLIPBackboneWithSVD)
                    self.backbone_norm = utils.ExportableImageNormalization(
                        mean=CLIP_MEAN, std=CLIP_STD
                    )
